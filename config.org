...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (set-frame-font "Inconsolata LGC-14" nil t)
...
(use-package window
  ...
  (setq display-buffer-alist
	'(
	  ...
	  ;; bottom side window
          ("\\*\\(Output\\|Register Preview\\|Flow Output\\|Completions\\).*"
           (display-buffer-in-side-window)
           (window-height . 0.30)
           (side . bottom)
           (slot . -1)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package jest
    :ensure t
    :config
    (setq jest-executable "yarn test --coverage=false --verbose=false")
