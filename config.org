...
(use-package replace
  ...
  (defun me/occur-browse-url-in-buffer ()
    "Run `eww' on a URL from the buffer using completion.
Also see `me/occur-url'."
    (interactive)
    (let ((matches nil))
      (save-excursion
        (goto-char (point-min))
  ...
  (defun me/occur-visit-or-list-urls (&optional arg)
    "Wrap `me/occur-visit-or-list-urls' and `me/occur-url'.
Meant to economise on key bindings."
    (interactive "P")
    (if arg
        (me/occur-url)
      (me/occur-browse-url-in-buffer)))

  :hook ((occur-mode-hook . hl-line-mode)
         (occur-mode-hook . (lambda ()
                              (toggle-truncate-lines t))))
  :bind (("M-s u" . me/occur-visit-or-list-urls)
         ("M-s M-o" . multi-occur)
         :map occur-mode-map
...
(use-package emacs
  :config
  (defun me/downward-list (&optional arg)
...
'backward'-facing command)."
  ...
  :bind ("C-M-d" . me/downward-list)) ; inverse of C-M-u
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package js
    ...
    :config
    (defun me/add-node-modules-path ()
    ...
    :hook (js-mode . me/add-node-modules-path))
